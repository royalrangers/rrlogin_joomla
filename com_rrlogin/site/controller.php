...
require_once JPATH_ROOT . '/components/com_rrlogin/helpers/password.php';
...
class RrloginController extends RrloginControllerParent
{
    ...
    public function auth()
    {
        ...
        $app->setUserState('com_rrlogin.action.data', $input->getString('action', ''));

        $redirect = JURI::base().'?option=com_rrlogin&task=check&plugin='.$plugin;
        ...
        if($this->config->get('service_auth', 0))
        {
            ...
            $url = 'http://api.royalrangers.network/index.php?action=token&site='.$service_site_id.'&pass='.$service_password.'&provider='.$plugin;
        }
        else if(JPluginHelper::isEnabled('rrlogin_auth', $plugin))
        {
            $dispatcher	= JDispatcher::getInstance();

            JPluginHelper::importPlugin('rrlogin_auth', $plugin);

            $url = $dispatcher->trigger('onRrloginAuth');
            $url = $url[0];
        }
        else{
    ...
    public function check()
    {
        ...
        if($this->config->get('service_auth', 0))
        {
            ...
            $url = 'http://api.royalrangers.network/index.php?action=get_user_data&site='.$service_site_id.'&pass='.$service_password.'&token='.$token;
            ...
            $this->rrlogin_id    = !empty($request->provider_id) ? $request->provider_id : '';
        ...
        else if(JPluginHelper::isEnabled('rrlogin_auth', $plugin))
        {
            ...
            $request = $dispatcher->trigger('onRrloginCheck');
            $request = $request[0];
            ...
            if (isset($request->first_name))
            {
                ...
                $this->email        = !empty($request->email) ? $request->email : '';
                $this->rrlogin_id    = !empty($request->id) ? $request->id : '';
        ...
        $app = JFactory::getApplication();
        $popup = $app->getUserState('com_rrlogin.popup', 'yes');
        $app->setUserState('com_rrlogin.popup', 'yes');
...
// 		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 2);
    ...
    public function setUserUserName()
    {
        ...
        {
            $name = strtolower($this->transliterate($this->first_name.'-'.$this->last_name.'-'.$this->provider));
        }
        else
        {
            $name = $this->email;
    ...
    private function deleteRrloginUser($rrlogin_id, $provider){
        $db = JFactory::getDbo();
        ...
        $query->where($db->quoteName('rrlogin_id') . ' = ' . $db->quote($rrlogin_id));
        ...
        $dispatcher->trigger('onBeforeRrloginDeleteRrloginUser',array($id));

        $query = $db->getQuery(true);
        $query->delete();
        $query->from($db->quoteName('#__rrlogin_users'));
        $query->where($db->quoteName('id') . ' = ' . $db->quote($id));
        $db->setQuery($query);
        $db->query();

        $dispatcher->trigger('onAfterRrloginDeleteRrloginUser',array($id));
        return true;
    }
    ...
    protected function storeUser()
    {
        ...
        {
            $app->setUserState(
                'com_rrlogin.provider.data',
                array(
                    'email' => $this->email,
                    'first_name' => $this->first_name,
                    'last_name' => $this->last_name,
                    'provider' => $this->provider,
                    'rrlogin_id' => $this->rrlogin_id,
                )
            );
        }
        ...
        $password = RrloginPasswordHelper::generatePassword($this->rrlogin_id, $this->provider, $secret);
        ...
        $this->storeRrloginUser($userId, $this->rrlogin_id, $this->provider);

        //вставка нового пользователя в таблицы других компонентов
        JPluginHelper::importPlugin('rrlogin_integration');
        $dispatcher = JDispatcher::getInstance();
        $dispatcher->trigger('onAfterRrloginStoreUser',array(JFactory::getUser($userId), $this->provider, $this->rawRequest));
...
//        $db->setQuery($query)->execute();
    ...
    protected function loginUser($id, $provider, $info=array())
    {
        ...
        JPluginHelper::importPlugin('rrlogin_integration');
        $dispatcher = JDispatcher::getInstance();
        $dispatcher->trigger('onBeforeRrloginLoginUser',array($user, $provider, $info));

        $password = RrloginPasswordHelper::generatePassword($this->rrlogin_id, $this->provider, $this->config->get('secret',''));
        ...
        $dispatcher->trigger('onAfterRrloginLoginUser',array($user, $provider, $info));
    ...
    public function displayRedirect($redirect='/', $popup=false, $msg = '', $msgType = 'message')
    {
        if($popup){
            $app = JFactory::getApplication();
            $app->setUserState('com_rrlogin.msg', $msg);
            $app->setUserState('com_rrlogin.msgType', $msgType);
            $session = JFactory::getSession();
            $redirect = base64_encode($redirect);
            $session->set('rrlogin_return', $redirect);
    ...
    {
        $session = JFactory::getSession();
        $session->set('rrlogin_errors', $error);
    ...
    public function sredirect()
    {
        ...
        $redirect = JRoute::_(base64_decode($session->get('rrlogin_return', '')), false);
        $session->clear('rrlogin_return');
        if ($error = $session->get('rrlogin_errors', null)) {
            $session->clear('rrlogin_errors');
    ...
    public function GetUserId()
    {
        ...
        $query->from($db->quoteName('#__rrlogin_users'));
        $query->where($db->quoteName('rrlogin_id') . ' = ' . $db->quote($this->rrlogin_id));
    ...
    public function GetRrloginStringId($rrlogin_id, $user_id, $provider)
    {
        // Initialise some variables
        $db = JFactory::getDbo();
        $query = $db->getQuery(true);
    ...
    public function join_mail()
    {
        ...
        $app = JFactory::getApplication();
        $appRedirect = $app->getUserState('com_rrlogin.return_url');
        $UserState = $app->getUserState('com_rrlogin.comparison_user.data');

        $msg = '';
        $user_id = $input->Get('user_id', 0, 'INT');
        $rrlogin_id = $input->Get('rrlogin_id', '', 'STRING');
        ...
        if (true === $app->login($credentials, $options)) {
            ...
            $app->setUserState('com_rrlogin.return_url', $appRedirect);
            $app->setUserState('com_rrlogin.comparison_user.data', $UserState);
            ...
            $this->deleteRrloginUser($rrlogin_id, $provider);
            //вносим данные в #__rrlogin_user
            $store = $this->storeRrloginUser($joomlaUserId, $rrlogin_id, $provider);
        ...
        {
            $app->setUserState('com_rrlogin.comparison_user.data', $UserState);
            $app->redirect(JRoute::_('index.php?option=com_rrlogin&view=linking_user', false));
        }
    }
    ...
    private function storeRrloginUser($user_id, $rrlogin_id, $provider){
        if(empty($user_id) || empty($rrlogin_id) || empty($provider)){
            return false;
        }
        JTable::addIncludePath(JPATH_COMPONENT . '/tables');
        $RrloginUser = &JTable::getInstance('rrlogin_users', 'RrloginTable');
        $RrloginUser->user_id = $user_id;
        $RrloginUser->rrlogin_id = $rrlogin_id;
        $RrloginUser->provider = $provider;
        $result = $RrloginUser->store();
        return $result;
    }
    ...
    public function check_mail()
    {
        ...
        $rrlogin_id =    $data['rrlogin_id'];
        $provider =     $data['provider'];
        $email =        $input->getString('email', '');
        $name =         $input->getString('name', '');
        $username =     $input->getString('username', '');

        require_once JPATH_SITE.'/components/com_rrlogin/controllers/validate.php';

        $validator = new RrloginControllerValidate();

        //валидация
        if(!$validator->validateEmail($email)){
            $msg  = JText::_('COM_SLOGIN_ERROR_VALIDATE_MAIL');
            $this->displayRedirect('index.php?option=com_rrlogin&view=mail', false, $msg, 'error');
        }
            ...
            if($this->config->get('collate_users', 0) == 1)
            {
                $data = array(
                    ...
                    'provider' => $provider,
                    'rrlogin_id' => $rrlogin_id,
                );
                $app->setUserState('com_rrlogin.comparison_user.data', $data);
                $this->displayRedirect('index.php?option=com_rrlogin&view=comparison_user', false, $msg, 'error');
            }
            else
            {
                $this->displayRedirect('index.php?option=com_rrlogin&view=mail', false, $msg, 'error');
            ...
            $this->email        = $email;
            $this->rrlogin_id    = $rrlogin_id;
            $this->provider     = $provider;
            $this->rawRequest   = $app->getUserState('com_rrlogin.provider.info');
            $this->network      = !empty($this->rawRequest->network) ? $this->rawRequest->network : '';

            $app->setUserState('com_rrlogin.reg_fields_edited', 1);
        ...
        if (!$rrloginUserId){
            ...
            if($this->config->get('query_email', 0)==1)
            {
                ...
                if(!$validator->validateEmail($this->email)){
                    $msg  = JText::_('COM_RRLOGIN_ERROR_VALIDATE_MAIL');
                    $this->displayRedirect('index.php?option=com_rrlogin&view=mail', $popup, $msg, 'error');
                }
                else if(!$validator->checkUniqueEmail($this->email)){
                    $msg = JText::_('COM_RRLOGIN_ERROR_NOT_UNIQUE_MAIL');
                    if($this->config->get('collate_users', 0) == 1)
                    {
                        $data = array(
                            'email' => $this->email,
                        ...
                        $app->setUserState('com_rrlogin.comparison_user.data', $data);
                        $this->displayRedirect('index.php?option=com_rrlogin&view=comparison_user', $popup, $msg, 'error');
                    }
                    else
                    {
                        $this->displayRedirect('index.php?option=com_rrlogin&view=mail', $popup, $msg, 'error');
                ...
                $this->email = (strpos($this->provider, '.') === false) ? $this->rrlogin_id.'@'.$this->provider.'.com' : $this->rrlogin_id.'@'.$this->provider;
            }
            ...
            if($freeEmail != $this->email){
                ...
                if($this->config->get('collate_users', 0) == 1){
                    ...
                    $app->setUserState('com_rrlogin.comparison_user.data', $data);
                    $return = 'index.php?option=com_rrlogin&view=comparison_user';
                    $msg = JText::_('COM_RLOGIN_MAIL_NOT_FREE');
            ...
            if($joomlaUserId > 0)
            {
                ...
                $model = parent::getModel('Linking_user', 'RrloginModel');

                if($app->getUserState('com_rrlogin.after_reg_redirect'))
                {
                    $return = base64_decode($app->getUserState('com_rrlogin.after_reg_redirect'));
                }
                else
                {
                ...
                $app->setUserState('com_rrlogin.return_url', $appReturn);
            }
        }
        else {   //или логинимся
	     ...
	     if($rrloginUserId){
           ...
           $app->setUserState(
            ...
            $this->loginUser($rrloginUserId, $this->provider, $this->rawRequest);
        }
        $this->displayRedirect($return, $popup, $msg);
    }

    /**  Слияние пользователей
     * @param null $rrlogin_id - ид выдаваемый провайдером
     * @param null $provider  - провайдер
     */
    protected function fusion($rrlogin_id= null, $provider= null, $popup=false)
    {
        $app = JFactory::getApplication();
        $app->setUserState('com_rrlogin.action.data', '');

        //ид текущего пользователя
        $user_id = JFactory::getUser()->id;

        if((int)$user_id == 0 || !$rrlogin_id || !$provider){
            return;
        }

        //удаляем старые записи пользователя из #__rrlogin_users
        $this->deleteRrloginUser($rrlogin_id, $provider);

        //добавляем новую запись пользователя в #__rrlogin_users
        $store = $this->storeRrloginUser($user_id, $rrlogin_id, $provider);

        $link = 'index.php?option=com_rrlogin&view=fusion';
    ...
    public function detach_provider()
    {
        ...
        $db = JFactory::getDbo();
        $query = $db->getQuery(true);
        $query->select($db->quoteName('rrlogin_id'));
        $query->from($db->quoteName('#__rrlogin_users'));
        $query->where($db->quoteName('user_id') . ' = ' . $db->quote($user_id));
        $query->where($db->quoteName('provider') . ' = ' . $db->quote($provider));
        $db->setQuery($query, 0, 1);
        $rrlogin_id = $db->loadResult();
    ...
    protected function queryEmail($popup=false)
    {
        ...
        $app->setUserState('com_rrlogin.provider.data', $data);
    }
    ...
    public function load_module_ajax(){
		...
		require_once JPATH_BASE.'/modules/mod_rrlogin/helper.php';
        ...
        $params->loadString(JModuleHelper::getModule('mod_rrlogin')->params);

        JFactory::getLanguage()->load('mod_rrlogin');
        ...
        $jll = (!modRrloginHelper::getalw($params))
