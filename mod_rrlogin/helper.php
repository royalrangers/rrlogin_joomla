...
class modRrloginHelper
{

    static function loadLinks(&$plugins, $add, $params){
        $plugins = RrloginProvidersHelper::loadProviderLinks($add);
 ...
 static function getalw(&$params){
	...
	foreach($allowedHost as $allowed){
		$allowed = modRrloginHelper::gethk($allowed, true);
		if(!empty($allowed)){
			...
			if(strpos($_SERVER['HTTP_HOST'], $site) !== false  && $extension == 'rrlogin')
			{
				$allow = true;
				break;
			}
