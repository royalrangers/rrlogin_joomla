...
require_once JPATH_ROOT.'/plugins/rrlogin_integration/profile/lib/profiles.php';
require_once JPATH_ROOT.'/plugins/rrlogin_integration/profile/lib/geo.php';
require_once JPATH_BASE.'/components/com_rrlogin/controller.php';

class plgRrlogin_integrationProfile extends JPlugin
{
    public function onAfterRrloginStoreUser($user, $provider, $info)
    {
        $this->createProfile($user, $provider, $info);
    }

    public function onAfterRrloginLoginUser($user, $provider, $info)
    {
        $conf = JComponentHelper::getParams('com_rrlogin');
        ...
        if($this->issetProfile($user, $provider))
        {
            if($serviceAuth)
            {
                $data = $this->rrloginServiceGetData($user, $provider, $info);
            }
            else
            {
    ...
    public function onBeforeRrloginDeleteRrloginUser($id)
    {
        $db = JFactory::getDbo();
    ...
    public function onBeforeRrloginDeleteUser($userId)
    {
        $db = JFactory::getDbo();
        $query = $db->getQuery(true);
        $query->select('*');
    ...
    private function deleteProfile($row)
    {
        ...
        $query = $db->getQuery(true);
        $query->select('avatar');
        $query->from($db->quoteName('#__plg_rrlogin_profile'));
    ...
    private function rrloginServiceGetData($user, $provider, $info){

        $data = new StdClass();
        $data->user_id =            $user->id;
        $data->rrlogin_id =          $info->provider_id;
    ...
    private function uloginGetData($user, $provider, $info){
        $data = new StdClass();
        $data->user_id = $user->id;
        $data->rrlogin_id = 'ulogin_' . $info->network . '_' . $info->uid;
        $data->provider = $provider;
    ...
    private function vkontakteGetData($user, $provider, $info)
    {
       ...
       private function rrnetworkGetData($user, $provider, $info)
    {
...
//        $data->picture = '';
    ...
    private function getGeoInfo(&$data){
        ...
        {
            $geo = new RrloginGeo(array('charset'=>'UTF-8', 'ip'=>$_SERVER["REMOTE_ADDR"]));
    ...
    private function createProfile($user, $provider, $info)
    {
        ...
        if (isset($data->picture)){
            ...
            $new_image = $provider . '_' . $data->rrlogin_id . '.jpg';
        ...
        $db = JFactory::getDbo();
        $q = $db->getQuery(true);
        $q->update('#__plg_rrlogin_profile');
        ...
        $table = new PlgRrloginProfilesTable($db);
        $table->load();
        $table->bind($data);
        $table->store();
    }
